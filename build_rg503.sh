#!/bin/bash
# Ensure this script runs from the repo root.
ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
cd "${ROOT_DIR}"

if [ -z "${DEBIAN_CODE_NAME}" ]; then
  export DEBIAN_CODE_NAME="trixie"
  echo "DEBIAN_CODE_NAME not set; defaulting to ${DEBIAN_CODE_NAME}. Use DEBIAN_CODE_NAME=<suite> to override."
fi
#exec 3>&1 4>&2
