...
BACKUP_FILE="/roms/backup/hoffmanosbackup.tar.gz"
LEGACY_BACKUP_FILE="/roms/backup/arkosbackup.tar.gz"
if [ ! -f "$BACKUP_FILE" ]; then
  if [ -f "$LEGACY_BACKUP_FILE" ]; then
    sudo mv "$LEGACY_BACKUP_FILE" "$BACKUP_FILE"
  else
    printf "\nNo hoffmanosbackup.tar.gz file was found in the"
    printf "\n/roms/backup folder.\n"
    printf "Please place the hoffmanosbackup.tar.gz file there\n"
...
printf "\nAre you sure you want to restore a backup of your HoffmanOS settings?\n"
printf "\nPress A to continue.  Press B to exit.\n"
while true
do
    Test_Button_A
    if [ "$?" -eq "10" ]; then
		LOG_FILE="/roms/backup/lasthoffmanosrestore.log"
		...
		if [ $? -eq 0 ]; then
			...
			do
				...
				if [ "$?" -eq "10" ]; then
					...
					sleep 1
					printf "\n The hoffmanosbackup.tar.gz file has been deleted from the /roms/backup folder.  Exiting..."
					sleep 3
					exit 0
				fi
				...
				Test_Button_B
				if [ "$?" -eq "10" ]; then
					printf "\n Exiting without deleting the hoffmanosbackup.tar.gz file from the /roms/backup folder." | tee -a "$LOG_FILE"
					printf "\033[0m" | tee -a "$LOG_FILE"
					sleep 3
					exit 0
				fi
