...
a10mini:
	$(info HoffmanOS will be built using the $(DEBIAN_CODE_NAME) release of Debian.)
...
clean:
	...
	sudo rm -rf Arkbuild Arkbuild32 Arkbuild-final hoffmanos_* main mnt odroidgoA-4.4.y HoffmanOS_* rg351 wget-*
	while losetup -a | grep -m 1 HoffmanOS; do sudo losetup -d "$$(losetup -a | grep HoffmanOS | cut -d ':' -f 1)"; done
	@echo "Done!"
