...
if [[ $(ls -l /${directory}/j2me/jdk/bin/java > /dev/null 2>&1 && echo $?) != 0 ]]; then
  if [ ! -f "/${directory}/j2me/jdk/${JDKDOWNLOAD}-linux_aarch64.tar.gz" ]; then
    ...
    if [ $? -ne 0 ]; then
      msgbox "Please connect to a wifi internet connection so the free JDK can be installed for this emulator.  You can also manually download and install the files per the HoffmanOS Emulator and Ports section for j2me from the HoffmanOS wiki."
      exit 1
    fi
