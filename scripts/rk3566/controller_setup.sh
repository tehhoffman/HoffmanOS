...
if [[ -f "${ra_config_paths[0]}/autoconfig/udev/${controller}.cfg" ]]; then
  if test ! -z "$(cat "${ra_config_paths[0]}/autoconfig/udev/${controller}.cfg" | grep -E "# (ArkOS|HoffmanOS)" | tr -d '\0')"
  then
    exit 0
  fi
fi
...
do
    echo "# HoffmanOS" > "${raconfigcreate}/autoconfig/udev/${controller}".cfg
